...
outFOLDER=/lustre19/expphy/volatile/halld/home/ehingerl/jpsi/trees/v8/varyCuts

mkdir -p ${outFOLDER}
for i in ${!Targets[@]}; do
  INPUTFILE=${inFOLDER}/${Targets_names[$i]}//e_e_p_X/e_e_p_X_*.root
  TREE=${outFOLDER}/tmp_data_tree_${Targets[$i]}.root
  root -q -b 'filter_cutVary_test.C("'${INPUTFILE}'","'${TREE}'","'${TreeNames[$i]}'")'&
done
...
#        root -q -b 'filter_cutVary.C("'${INPUTFILE}'","'${TREE}'","'${TreeNames[$i]}'")'&
#      fi
#    done
#  done
#done
