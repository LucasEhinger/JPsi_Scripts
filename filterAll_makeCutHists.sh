...
#  root -q -b 'filterAll_makeCutHists.C("'${INPUTFILE}'","'${HISTFILE}'","'${TreeNames[$i]}'","'${trackShower_sel}'","'${PreBCalCut}'","'${sigma_minus}'","'${sigma_plus}'","'${EMissCut}'")'
#done
  ...
  for MFSRC in ${MFSRCStrings[@]}; do
    for i in ${!TargetsSIM[@]}; do
      ...
      then
        ...
        HISTFILE=${workFolder}/hist2_cutflow_DSelector_${TargetsSIM[$i]}_${MFSRC}_helicity${isMixed}.root
        root -q -b 'filterAll_makeCutHists.C("'${INPUTFILE}'","'${HISTFILE}'","'${TreeNames[$i]}'","'${trackShower_sel}'","'${PreBCalCut}'","'${sigma_minus}'","'${sigma_plus}'","'${EMissCut}'")'
      fi
    done
  done
