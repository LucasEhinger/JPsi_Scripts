...
for j in ${!FOLDERS[@]}; do
  tmp1=${FOLDERS[$j]}
  inFolder=${tmp1%data*}
  outFolder=${workFolder}${inFolder##*trees}/simHists/resolution/
  ...
  for isMixed in ${mixedStrings[@]}; do
    for MFSRC in ${MFSRCStrings[@]}; do
      for i in ${!TargetsSIM[@]}; do
        ...
        TREE=${outFolder}/hist_resolution_DSelector_${TargetsSIM[$i]}_${MFSRC}_helicity${isMixed}.root
        if [[ ${TargetsSIM[$i]} != "2H" || ${MFSRC} != "SRC" ]]
        then
          root -q -b 'make_sim_resolution_hists.C("'${INPUTFILE}'","'${TREE}'","'${TreeNames[$i]}'")'
        fi
      done
    done
  done
